...
public class RegistryRecoveryDataStore implements UserRecoveryDataStore {
    ...
    public static final String REGISTRY_SEARCH_FIELD_PROPERTY_NAME = "propertyName";
    public static final String REGISTRT_SEARCH_FIELD_RIGHT_PROPERTY_VALUE = "rightPropertyValue";
    public static final String REGISTRY_SEARCH_FIELD_RIGHT_OP = "rightOp";
    public static final String REGISTRY_SEARCH_FIELD_RIGHT_OP_EQ = "eq";
    ...
    private void deleteOldConfirmationCodesByRegistrySearch(Registry registry, String username, String
        ...
        if (searchResults != null && !ArrayUtils.isEmpty(searchResults)) {
            ...
            for (ResourceData resource : searchResults) {
                ...
                if (resourcePath != null && resourcePath.contains(confimationCodePath)) {
                    if (log.isDebugEnabled()) {
                    ...
                    try {
                        ...
                        log.error("Error while deleting the old confirmation code, for user: " + username + " at " +
                                "resource path: " + resourcePathRelativeToConfigRegistry, e);
                    }
                }
            }
        } else {
    ...
    private void deleteOldConfirmationCodesByResourceIteration(Registry registry, String username, String
        ...
        try {
            collection = (Collection) registry.get(confimationCodePath.toLowerCase());
        } catch (RegistryException e) {
            log.error("Error while deleting the old confirmation code for user: " + username + ". Cannot find " +
                    "resource collection at resource path: " + confimationCodePath, e);
        }
        ...
        if (collection != null) {
            ...
            try {
                ...
                log.error("Error while deleting the old confirmation code for user: " + username + " at resource " +
                        "" + "path: " + confimationCodePath, e);
            }
        } else {

            if (log.isDebugEnabled()) {
                log.debug("No registry resource found at path: " + confimationCodePath);
            }
        }
    }

    private void deleteRegistryResource(Registry registry, String resourcePathRelativeToRegistry) throws
            RegistryException {
